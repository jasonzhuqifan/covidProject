...
Then, the time-inhomogeneous CTMC density over the observation period is 
...
### The Hidden Markov Model Step 

Rewriting the probability of the infectious people got tested
...
### The Discrete Time Skeleton Step 
We first sample the disease state at the intermediate event times when the CTMC rates change, and then sampling the full path within each interevent interval. Consider an interobservation interval, $I_t =[t_{l-1}, t_l]$, containing interevent intervals whose endpoints are given by times $t_{l-1}=\tau_{l,0}^{(-j)}<\tau_{l,1}^{(-j)}<... <\tau_{l,n-1}^{(-j)}<\tau_{l,n}^{(-j)} \equiv t_l$ . Let $\tau =\tau_{l,1}^{(-j)}$ and $x_i=x_j(\tau_{l,1}^{(-j)})$, sample $X_j$ at each intermediate event time, beginning at $\tau_1$, from the discrete distribution with masses.



$$Pr(X_j(\tau_i) = x_i | X_j(\tau_{i-1} = x_{i-1}, X_j(\tau_n) = x_n)) \
= \frac{[P^{(j)(\tau_{i-1}, \tau_i)}]_{x_{i-1}, x_i}[\prod_{k=1}^{n-1} P^{(j)(\tau_k, \tau_{k+1})}]_{x_i, x_n}}{[\prod_{k=1}^{n-1}P^{(j)}(\tau_k, \tau_{k+1})]_{x_{i-1}, x_n}}$$
  
### Event Time Step
The final step in constructing a subject-path is to sample the exact infection and recovery times given the discrete sequence of states obtained in the previous two steps. When fitting the SIR model, we choose to use modified rejection sampling, a modification of Gillespie’s direct algorithm that explicitly avoids simulating constant paths. The SEIR model uses a uniformization-based sampling for a more robust sampling of paths in intervals with multiple transitions.
...
### Initializing the Collection of Subject Path
The initialization of the subject-paths starts with using Gillespie’s direct algorithm to simulate paths and iterates the algorithm until we have found one probable collection with nonzero probability.


### Parameter Updates
One MCMC iteration updates subject-path and parameters consecutively, this is because only updating subject-path or parameters will cause high auto-correlation. We then use Gibbs sampling to draw new parameter values from their univariate full conditional distributions. 
...
gibbs_kernel_SEIR <- function(epimodel) {
  ...
  #' @references (Gibbs template)[ https://github.com/fintzij/BDAepimodel/blob/f73daebff0d46bbd7e68af1429f37b4665fae92b/R/gibbs_template.R]
...
best_parmas_finder <- function(params, dat, SEIR=FALSE, max_iter=100, niter=100, thres=2e-5, pop_size=2000){
  #' Calculate the average for each columns
  #'
  #' @param param parameter matrix to calculate avarege
  #' @return calcualte average with 10 burn-in values
...
initial_params_1500 <- list(beta=0.00015, mu=0.26, rho=0.09)
...
```{r eval=FALSE, echo=FALSE}
initial_params <- list(beta=0.00040, gamma=0.10, mu=0.03, rho=0.010)
epimodel <- best_parmas_finder(initial_params, dat=waterloo_sampled, 
                               SEIR=TRUE, thres=1e-5, pop_size=1500)
```


```{r eval=FALSE, echo=FALSE}
initial_params <- list(beta=0.00040, gamma=0.10, mu=0.03, 
                       rho=0.010, popsize=1500)
...
Although using the full population will result in a long run-time, the research paper conducted an experiment under population misspecification. Given different population size, the posterior inferences about the epidemic dynamics are not substantially affected. The estimated parameters under 2000 population size can relatively accruate for the full population size.

One other drawback is we cannot find a set of suitable parameters with completely randomly generated parameters, we have to start with an educated guess which will at least give a non-zero probability after the initialization step.
